...
public class SalesAmountServiceImpl implements SalesAmountService {
    ...
    public int getSalesAmountForDate(Date date) {
        ...
        try (FileInputStream fileInputStream = new FileInputStream(filePath);
            ...
            for (int rowIndex = 1; rowIndex <= sheet.getLastRowNum(); rowIndex++) {
                ...
                Cell countCell = row.getCell(5); // Count 컬럼 (Index 3)
                if (countCell != null && countCell.getCellType() == CellType.NUMERIC) {
                    totalSalesAmount += (int) countCell.getNumericCellValue(); // Count 값을 합산
                }
            }
